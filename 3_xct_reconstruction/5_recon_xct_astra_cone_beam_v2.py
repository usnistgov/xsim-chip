...
# Cone beam detector parameters
det_spacing_x = 0.125 # (mm)
det_spacing_y = 0.125 # (mm)
det_row_count = 1001 # (pixels)
det_col_count = 1201 # (pixels)

# Cone beam XCT positions (both should be positive)
source_origin = 30.0  # [mm]
origin_det = 907.3    # [mm]

# The width of each pixel in mm for each projection image
pix_size = 0.00400059 # [mm/pixel]
...
# Initialize the projection object containing all of our projection images.
proj_geom = astra.create_proj_geom('cone', det_spacing_x/pix_size, det_spacing_y/pix_size,
    det_row_count, det_col_count, angle_arr, source_origin/pix_size, origin_det/pix_size)

projection_id = astra.data3d.create('-proj3d', proj_geom, img_stack)

# Create a volume object. This will store our reconstruction.
print("\nCreating reconstruction data object...")
vol_geom = astra.create_vol_geom([det_col_count, det_col_count, det_row_count])
#vol_geom = astra.create_vol_geom([det_col_count/2, det_col_count/2, det_row_count/2])

reconstruction_id = astra.data3d.create('-vol', vol_geom)

# Perform reconstruction using the 3D FDK algorithm
...
recon_imgs = astra.data3d.get(reconstruction_id)
...
recon_imgs_top = np.moveaxis(recon_imgs, [0, 1, 2], [-1, 0, 1])
